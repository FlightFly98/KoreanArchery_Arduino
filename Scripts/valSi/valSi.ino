...
const int udpPort = 12348;
bool ipAcquired = false;

const int valSensorPin = D1;

void setup() {
  Serial.begin(115200);
  
  pinMode(valSensorPin, INPUT);
...
void sendSensorData()
{
  int valValue = digitalRead(valSensorPin);
  char buffer[256];
  if (valValue == LOW) {
    // 충격 감지 시 데이터 전송
    snprintf(buffer, sizeof(buffer), "ValSi");
    Udp.beginPacket(udpAddress, udpPort);
    Udp.write(buffer);
    Udp.endPacket();

    // 디버깅을 위해 Serial 출력
    Serial.println("ValSi detected!");
